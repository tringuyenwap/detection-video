...
class SsdFpn:
    ...
    def __init__(self, confidence_threshold, config, is_bgr=True):
        ...
        self.network_input_size = (640, 640)
        self.num_classes = 90
        self.graph_name = 'ssdfpn'
        self.session = read_graph_and_init_session('./models/ssd_resnet50_v1_fpn/frozen_inference_graph.pb',
    ...
    def __run_detection_network(self, images):
        num_detections, detection_scores, detection_boxes, detection_classes =  \
            self.session.run([self.session.graph.get_tensor_by_name(self.graph_name + '/num_detections:0'),
        ...
        return num_detections, detection_scores, detection_boxes, detection_classes

    def __preprocessing_image_for_detection_network(self, image):
        image = cv.resize(image, self.network_input_size)
        if self.is_bgr:
    ...
    def get_detections_batch(self, images):
        ...
        num_detections, detection_scores, detection_boxes, detection_classes = self.\
