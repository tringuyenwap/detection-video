...
from discriminator.fusion.data_set_reader import create_readers_split
import utils
...
class Experiment:
    ...
    def __init__(self, prefix_checkpoint, is_testing=False, num_channels=2, learning_rate_init=10 ** -3, num_epochs=30, batch_size=64):
        ...
        self.checkpoint_folder = os.path.join(args.CHECKPOINTS_BASE, "network_disc_fusion_%s" % prefix_checkpoint)
        self.IS_RESTORE = tf.train.latest_checkpoint(self.checkpoint_folder) is not None
        self.inputs_recon = tf.compat.v1.placeholder(np.float32, [None, 64, 64, num_channels])
        # if prefix_checkpoint.find('app') != -1:
        #     self.inputs_latent = tf.compat.v1.placeholder(np.float32, [None, 8, 8, 16])
        # else:
        self.inputs_latent = tf.compat.v1.placeholder(np.float32, [None, 8, 8, 16])
        ...
        # if prefix_checkpoint.find('app') != -1:
        #     self.logits = model.model_fusion_diff_and_latent(self.inputs_recon, self.inputs_latent, self.is_training)
        # else:
        self.logits = model.model_fusion_diff_and_latent(self.inputs_recon, self.inputs_latent, self.is_training)
    ...
    def predict(self, image_recon, latent):
        
        if self.prefix.find('app') != -1:
            image_recon = np.expand_dims(np.expand_dims(image_recon, axis=2), axis=0)
        else:
            image_recon = np.expand_dims(image_recon, axis=0)

        latent = np.expand_dims(latent, axis=0)

        predictions = self.sess.run(self.logits, feed_dict={self.inputs_latent: latent, self.inputs_recon: image_recon,
    ...
    def fit(self, reader_train):
        ...
        for iter in range(iters):
            batch_latent, batch_recon, batch_y = reader_train.next_batch(self.batch_size)
            _, c, predictions, _ = self.sess.run([self.optimizer, self.cost, self.logits, self.global_step],
                    feed_dict={self.inputs_latent: batch_latent, self.inputs_recon: batch_recon,
                               self.targets_: batch_y, self.is_training: True})
            total_loss += np.sum(c)
            predictions = (predictions > 0) * 1
            un_norm_acc += np.sum(np.round(predictions) == batch_y)
            # if iter % 100 == 0:
            #     print(np.mean(np.round(predictions) == batch_y))
            # pdb.set_trace()
    ...
    def eval(self, reader, return_predicted_labels=False):
        ...
        for iter in range(iters):
            batch_latent, batch_recon, batch_y = reader.next_batch(self.batch_size)
            c, predictions = self.sess.run([self.cost, self.logits],
                    feed_dict={self.inputs_recon: batch_recon, self.inputs_latent: batch_latent
                        , self.targets_: batch_y, self.is_training: False})
            total_loss += np.sum(c)
            predictions = (predictions > 0) * 1            
...
def train(prefix_ae):
    ...
    reader_train, reader_val = create_readers_split(prefix_ae, num_channels=num_channels)
